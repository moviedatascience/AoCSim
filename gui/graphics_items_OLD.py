...
class VertexItem(QGraphicsEllipseItem):
    ...
    def __init__(self, x, y, radius, parent_polygon):
        ...
        Parameters:
            ...
            parent_polygon (PolygonItem): The parent polygon item.
        """
        ...
        self.parent_polygon = parent_polygon  # Store the parent polygon
        ...
        if change == QGraphicsItem.ItemPositionChange and self.parent_polygon.edit_mode:
            ...
            if new_pos != self.pos():
                ...
                try:
                    ...
                    self.parent_polygon.updateVertex(self, QPointF(new_pos.x(), new_pos.y()))  # Update the parent polygon with the new position
                finally:
...
class PolygonItem(QGraphicsPolygonItem):
    ...
    def createVertexItems(self):
        ...
        for point in self.polygon():
            vertex_item = VertexItem(point.x(), point.y(), self.vertex_radius, self)  # Create a new vertex item
    ...
    def updateVertex(self, vertex_item, new_pos):
        ...
        Parameters:
            vertex_item (VertexItem): The vertex item to update.
            new_pos (QPointF): The new position of the vertex.
        """
        old_pos = vertex_item.pos()  # Get the old position of the vertex
        try:
            index = self.vertex_items.index(vertex_item)  # Find the index of the vertex item
            polygon = self.polygon()  # Get the current polygon
            if polygon.at(index) != new_pos:
                polygon.replace(index, new_pos)  # Update the position in the polygon
                self.setPolygon(polygon)  # Set the updated polygon
                self.updateVertexItems()  # Update all vertex items
                self.shared_vertex_manager.update_vertex((old_pos.x(), old_pos.y()), (new_pos.x(), new_pos.y()))  # Update the shared vertex manager

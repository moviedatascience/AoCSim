...
def create_voronoi_overlay(image_path, output_path, points, mask):
    ...
    for pointidx, region_index in enumerate(vor.point_region):
        ...
        # If region has -1 we have infinite vertices
        if -1 in vertices:
            inf_vertex_indices = [i for i, vertex in enumerate(vertices) if vertex == -1]
            finite_vertices = [vor.vertices[i] for i in vertices if i != -1]

            # Construct a line segment for each infinite edge, extending from the finite vertex to a point outside the plot
            for inf_vertex_index in inf_vertex_indices:
                point = points[pointidx]
                finite_vertex = finite_vertices[inf_vertex_index]
                t = point - finite_vertex
                t /= np.linalg.norm(t)

                # Extend the line segment to the edges of the plot
                line = LineString([finite_vertex, finite_vertex + t*2000])
                intersected_line = bounding_box.intersection(line)

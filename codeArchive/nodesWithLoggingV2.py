...
def get_missing_regions_from_log(log_file_path):
    missing_regions = []
...
# Read missing regions from the log
log_file_path = 'voronoi_log.log'
missing_regions = get_missing_regions_from_log(log_file_path)
...
for region_index in missing_regions:
    adjusted_index = region_index - 1  # Adjust for zero-indexing
    if adjusted_index in vor.point_region:
        region = vor.regions[vor.point_region[adjusted_index]]
        if not -1 in region and region:  # Check for valid region
            polygon = [vor.vertices[i] for i in region]
            ax.fill(*zip(*polygon), color='red', alpha=0.4)
            logging.debug(f"Highlighting region {region_index}")  # Debug log
ax.invert_yaxis()

...
class SimulationApp(QtWidgets.QMainWindow):
    ...
    def load_and_draw_regions(self):
        points = self.load_voronoi_points()
        if points:
            voronoi_regions = voronoi_diagram(MultiPoint(points))
            for region in voronoi_regions:
                if not region.is_empty and isinstance(region, shapely.geometry.Polygon):
                    qpoints = [QPointF(*coord) for coord in region.exterior.coords]
                    polygon = QtGui.QPolygonF(qpoints)
                    polygon_item = HoverablePolygonItem(polygon)
                    self.mapView.scene.addItem(polygon_item)

    def load_voronoi_points(self):
        points = []
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute("SELECT x_REAL, y_REAL FROM regions")

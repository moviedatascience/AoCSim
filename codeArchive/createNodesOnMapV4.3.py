...
def compute_efficient_land_centroid(polygon, mask, width, height, region_id=None):
    ...
    If the standard method fails (often in edge regions), fall back to using the geometric center.
    """
    ...
    if len(x_indices) == 0 or len(y_indices) == 0:
        print(f"Region {region_id} has no overlap. Using fallback methods.")
        
        # Fallback 1: Geometric center of the polygon's vertices
        x_center = np.mean([v[0] for v in polygon.exterior.coords])
        y_center = np.mean([v[1] for v in polygon.exterior.coords])

        # Additional fallback: Midpoint of the bounding box
    ...
    try:
        with sqlite3.connect(db_path) as connection:
            ...
            for idx, region_json in regions_to_store.items():
                # Get the centroid for the current region
                centroid = points[idx]

...
class HoverablePolygonItem(QGraphicsPolygonItem):
    def __init__(self, polygon, region_id=None, parent=None):
        ...
        self.edit_mode = False  # Track whether in edit mode
        self.setBrush(self.normal_brush)
        self.setPen(QPen(Qt.black, 2))

    def hoverEnterEvent(self, event):
        if not self.edit_mode:  # Highlight normally if not in edit mode
            self.setBrush(self.hover_brush)
        # In edit mode, you may want to add logic here for highlighting boundaries or vertices
        super().hoverEnterEvent(event)
    
    def hoverLeaveEvent(self, event):
        if not self.edit_mode:  # Remove highlighting if not in edit mode
            self.setBrush(self.normal_brush)
        # In edit mode, you may need to add logic here for de-highlighting boundaries or vertices
        super().hoverLeaveEvent(event)

    def setEditMode(self, mode):
        self.edit_mode = mode
        if self.edit_mode:
            self.setBrush(Qt.transparent)  # In edit mode, remove normal highlighting
        else:
            self.setBrush(self.normal_brush)  # Reset to normal mode when exiting edit mode
...
class MapGraphicsView(QtWidgets.QGraphicsView):
    def __init__(self, parent=None):
        ...
        self.map_pixmap = QtGui.QPixmap(config.get('map_image_path'))
    ...
    def setEditMode(self, editMode):
        # Iterate through all HoverablePolygonItem instances and set their edit mode
...
class CollapsibleSidebar(QWidget):
    editModeChanged = QtCore.pyqtSignal(bool)  # It's good practice to define signals at the class level
    ...
    def __init__(self, parent=None):
        ...
        self.closeButton.setIcon(QIcon(config.get('close_icon_path')))  # Close icon
        self.closeButton.setFixedSize(50, 50)  # Adjust size as needed
        self.closeButton.clicked.connect(self.toggle)

        # Initialize the contentWidget and contentLayout before adding any buttons
        self.contentWidget = QWidget()
        self.contentLayout = QVBoxLayout(self.contentWidget)  # Define contentLayout before adding widgets to it

        # Adding new button for editing region boundaries
        ...
        self.layout.addWidget(self.contentWidget)  # Add contentWidget after it's fully set up
    ...
    # New method to handle edit mode toggle
    def toggleEditMode(self):
        self.editMode = not self.editMode  # Toggle edit mode state
        if self.editMode:
            # Entering edit mode
        ...
        self.editModeChanged.emit(self.editMode)  # Emit signal with the new mode state

...
def update_database_with_masks(db_path, regions_to_store, masks):
    ...
    for region_id, mask_data in masks.items():
        ...
        cur.execute("SELECT region_id FROM regions WHERE region_id = ?", (region_id,))
        exists = cur.fetchone()
        if exists:
            # Update existing record
            cur.execute("UPDATE regions SET vertices = ?, mask = ? WHERE region_id = ?", 
                        (vertices, mask_data, region_id))
        else:
            # Insert new record

...
def cap_unbounded_region(region, voronoi, bbox):
    """
    Cap an unbounded Voronoi region within the given bounding box.
    """
    new_vertices = []
    for vertex_index in region:
        if vertex_index >= 0:
            # For valid vertex indices, use the corresponding Voronoi vertex
            new_vertices.append(tuple(voronoi.vertices[vertex_index]))
            continue

        # Find the points where the Voronoi ridge intersects the bounding box
        point1, point2 = voronoi.ridge_vertices[region.index(vertex_index)]
        if point1 == -1 or point2 == -1:
            continue  # Skip if ridge points are invalid

        line = LineString([voronoi.vertices[point1], voronoi.vertices[point2]])
        intersection = line.intersection(bbox)
        if not intersection.is_empty:
            if isinstance(intersection, LineString):
                # If the intersection is a line, take its endpoints
                new_vertices.extend(list(intersection.coords))
            elif isinstance(intersection, Point):
                # If the intersection is a point, use it directly
                new_vertices.append((intersection.x, intersection.y))

    if len(new_vertices) < 3:
        return None  # Not enough vertices to form a polygon
...
for iteration in range(num_iterations):
    ...
    for idx, point in enumerate(points):
        ...
        if -1 in region:
            # Handle unbounded region
            polygon = cap_unbounded_region(region, vor, bounding_box)
            if polygon is None or polygon.is_empty:
                print(f"Region {idx} is unbounded and could not be capped.")
                continue
        else:
            # Handle bounded region
            polygon = Polygon([vor.vertices[i] for i in region if i != -1])

        # Clip the polygon to the map bounds
        clipped_polygon = clip_to_map_bounds(polygon)
        if not clipped_polygon:
            print(f"Region {idx} has no valid clipped polygon.")
            continue

        # Compute centroid and update points
        ...
        if centroid:
            ...
            print(f"Region {idx} processed and added to store.")
        else:
            new_points[idx] = point

...
class HoverablePolygonItem(QGraphicsPolygonItem):
    def __init__(self, polygon, region_id=None, parent=None):  # Added region_id parameter
        super().__init__(polygon, parent)
        self.region_id = region_id  # Store region_id as an attribute
        self.setAcceptHoverEvents(True)
        self.normal_brush = QBrush(Qt.transparent)
        self.hover_brush = QBrush(QColor(255, 255, 0, 100))  # Semi-transparent yellow
        self.setBrush(self.normal_brush)
        self.setPen(QPen(Qt.black, 2))  # Black border
    
    def hoverEnterEvent(self, event):
        print(f"Hovered over a region {self.region_id}") # Ensure you have a region_id attribute
        self.setBrush(self.hover_brush)
        super().hoverEnterEvent(event)  # Call the parent class method
    
    def hoverLeaveEvent(self, event):
        print("Left a region")
        self.setBrush(self.normal_brush)
        super().hoverLeaveEvent(event)  # Call the parent class method
...
class SimulationApp(QtWidgets.QMainWindow):
    ...
    def showEvent(self, event):
        ...
        # Load and draw regions after the main window is displayed

...
def create_voronoi_overlay(image_path, output_path, points, mask):
    ...
    for pointidx, region_index in enumerate(vor.point_region):
        ...
        if -1 in vertices:
            ...
            # Construct a line segment for each infinite edge
            for v in vertices:
                if v == -1:
                    # Find corresponding ridges for the infinite vertex
                    ridges = [ridge for ridge in vor.ridge_vertices if -1 in ridge]
                    for ridge in ridges:

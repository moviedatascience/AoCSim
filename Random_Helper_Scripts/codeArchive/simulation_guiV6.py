...
from PyQt5.QtWidgets import QGraphicsPolygonItem, QGraphicsPixmapItem, QGraphicsRectItem
from PyQt5.QtGui import QBrush, QColor, QPen, QPixmap, QPainterPath
...
class MapGraphicsView(QtWidgets.QGraphicsView):
    ...
    def draw_grid_regions(self):
        map_width = self.scene.width()
        map_height = self.scene.height()
        num_regions = 100
        grid_size = int(np.sqrt(num_regions))

        for x in range(0, int(map_width), int(map_width // grid_size)):
            for y in range(0, int(map_height), int(map_height // grid_size)):
                rect = QRectF(x, y, map_width // grid_size, map_height // grid_size)
                region_item = HoverableRectItem(rect)
...
class SimulationApp(QtWidgets.QMainWindow):
    ...
    def showEvent(self, event):
        ...
        self.mapView.draw_grid_regions()

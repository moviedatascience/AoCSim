...
def create_voronoi_overlay(image_path, output_path, points, mask):
    ...
    for pointidx, region_index in enumerate(vor.point_region):
        ...
        if -1 in vertices:
            new_vertices = [vor.vertices[v] for v in vertices if v >= 0]  # Finite vertices
            for v in vertices:
                if v == -1:
                    ...
                    for p1, p2 in ridge_points:
                        if pointidx not in (p1, p2):
                            continue
                        other_point = p2 if p1 == pointidx else p1
                        if other_point < 0:
                            continue
                        # Calculate the direction vector (`t`) as a float
                        t = points[other_point].astype(float) - points[pointidx].astype(float)
                        t /= np.linalg.norm(t)

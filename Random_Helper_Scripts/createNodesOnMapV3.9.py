...
def create_voronoi_overlay(image_path, output_path, points, mask):
    ...
    for pointidx, region in enumerate(vor.point_region):
        ...
        if -1 in vertices:
            # Identify the ridge between the point and its finite neighbor
            ridge_vertices = [vor.vertices[i] for i in vertices if i != -1]
            # Find the direction vector of the ridge
            tangent = ridge_vertices[1] - ridge_vertices[0]
            tangent /= np.linalg.norm(tangent)
            # Find a point far away in the direction of the ridge
            far_point = ridge_vertices[0] + tangent * np.max([map_width, map_height])
            # Add the far point to the region vertices
            vertices = [i for i in vertices if i != -1] + [far_point.tolist()]
            
        # Create the polygon for this region
        region_poly = Polygon([vor.vertices[v] for v in vertices if v >= 0])
